...
async function showRateNotification(context: vscode.ExtensionContext, reason: 'install' | 'firstCommand'): Promise<void> {
  const config = vscode.workspace.getConfiguration();
  const rateEnabled = config.get<boolean>('mirrorYourDevice.rateNotification.enabled', true);
  ...
  const userDecision = context.globalState.get<string>(USER_DECISION_KEY);
  ...
  const alreadyShown = context.globalState.get<boolean>(notificationKey, false);
  if (alreadyShown) {
    return;
  }
...
async function checkInstallDateNotification(context: vscode.ExtensionContext): Promise<void> {
  const config = vscode.workspace.getConfiguration();
  const daysAfterInstall = config.get<number>('mirrorYourDevice.rateNotification.daysAfterInstall', 7);
  
  // Get or set install date
  let installDate = context.globalState.get<number>(INSTALL_DATE_KEY);
  if (!installDate) {
...
async function checkFirstCommandNotification(context: vscode.ExtensionContext): Promise<void> {
  const config = vscode.workspace.getConfiguration();
  const showAfterFirstCommand = config.get<boolean>('mirrorYourDevice.rateNotification.showAfterFirstCommand', true);
  
  if (!showAfterFirstCommand) {
    return;
  }

  const firstCommandExecuted = context.globalState.get<boolean>(FIRST_COMMAND_EXECUTED_KEY, false);
...
async function listAdbDevices(adbPath: string, filterStatus: boolean): Promise<AdbDevice[]> {
  try {
    const { stdout } = await execAsync(`${adbPath} devices -l`);
  const lines = stdout.split(/\r?\n/).filter((l: string) => l.trim().length > 0);
    const devices: AdbDevice[] = [];
    for (const line of lines) {
      if (/^List of devices attached/.test(line)) continue;
      const parts = line.trim().split(/\s+/);
      if (parts.length >= 2) {
        const id = parts[0];
        const status = parts[1];
        if (filterStatus && status !== 'device') continue;
...
async function checkDependencies(adbPath: string, scrcpyPath: string): Promise<{ adb: boolean; scrcpy: boolean; }> {
  const adbOk = await binaryExists(adbPath);
  const scrcpyOk = await binaryExists(scrcpyPath);
  return { adb: adbOk, scrcpy: scrcpyOk };
}

async function showDependencyFeedback(adbOk: boolean, scrcpyOk: boolean) {
  if (!outputChannel) outputChannel = vscode.window.createOutputChannel('Mirror Your Device');
  ...
  if (!adbOk) missing.push('adb');
  if (!scrcpyOk) missing.push('scrcpy');
...
async function runScrcpy(scrcpyPath: string, deviceId: string, statusItem: vscode.StatusBarItem) {
  if (!outputChannel) {
  ...
  try {
    const proc = spawn(scrcpyPath, args, { stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  statusItem.text = `$(device-camera) scrcpy ${deviceId}: running`;
    setTimeout(() => { if (statusItem.text.includes(deviceId)) statusItem.hide(); }, 5000);
  } catch (e: unknown) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const config = vscode.workspace.getConfiguration();
  const showCheck = config.get<boolean>('scrcpy.showDependencyCheckOnStartup', true);
  if (showCheck) {
    const adbPath = config.get<string>('scrcpy.adbPath', 'adb');
    const scrcpyPath = config.get<string>('scrcpy.scrcpyPath', 'scrcpy');
  ...
  const disposable = vscode.commands.registerCommand('scrcpy.mirrorDevice', async () => {
    ...
    const adbPath = config.get<string>('scrcpy.adbPath', 'adb');
    const scrcpyPath = config.get<string>('scrcpy.scrcpyPath', 'scrcpy');
    const filterStatus = config.get<boolean>('scrcpy.filterStatus', true);
  ...
  const depCmd = vscode.commands.registerCommand('scrcpy.checkDependencies', async () => {
    ...
    const adbPath = cfg.get<string>('scrcpy.adbPath', 'adb');
    const scrcpyPath = cfg.get<string>('scrcpy.scrcpyPath', 'scrcpy');
